...
public class FacePanel extends JPanel implements Runnable {
	...
	private static final int DELAY = 100; // time (ms) between redraws of the
											// panel
	...
	private static final String CROSSHAIRS_FNM = "res/crosshairs.png";

	// for saving a detected face image
	private static final String FACE_DIR = "trainingImages";
	private static String personFileName = "face";
	...
	private int fileCount = 0; // used for constructing a filename for saving a
								// face
	...
	/*
	 * make sure there's a SAVE_DIR directory, and backup any images in there by
	 * prefixing them with "OLD_"
	 */
	{
		...
		if (saveDir.exists()) { // backup any existing files
			...
			if (listOfFiles.length > 0) {
				// System.out.println("Backing up files in " + FACE_DIR);
						...
						try {
							System.out
									.println(file
											+ ":"
											+ file.substring(0,
													personFileName.length())
											+ ":" + personFileName);

							if (file.substring(0, personFileName.length())
									.equals(personFileName)) {
								String numString = file.substring(
										personFileName.length(),
										file.length() - 4);
			...
			System.out.println("Creating directory: " + FACE_DIR);
	...
	// return an image
	{
		...
		try {
			file = new File(imFnm);
			i++;
			image = ImageIO.read(file); // read in as an image
			i++;
			System.out.println("Reading image " + imFnm);
		} catch (Exception e) {
			System.out.println(i + "Could not read image from " + imFnm + "\n"
	 ...
	 */
	{
		...
		System.out.println(" " + CAMERA_ID + ": "
				+ videoInput.getDeviceName(CAMERA_ID));
		if (grabber == null)
			return;
		System.out.println(" ->" + CAMERA_ID + ": "
				+ videoInput.getDeviceName(CAMERA_ID));
	...
	private FrameGrabber initGrabber(int ID) {
		...
		System.out.println(ID + " Initializing grabber for "
				+ videoInput.getDeviceName(ID) + " ...");
		System.out.println(" cnccn " + CAMERA_ID + ": "
				+ videoInput.getDeviceName(CAMERA_ID));
	 ...
	 */
	{
		...
		if (snapIm != null) {
			...
			g2.drawImage(snapIm.getBufferedImage(), 0, 0, this); // draw the
																	// snap
			...
			g2.drawString("Loading from camera " + CAMERA_ID + "...", 5,
					HEIGHT - 10);
		}
	...
	/*
	 * Terminate run() and wait for it to finish. This stops the application
	 * from exiting until everything has finished.
	 */
	{
		isRunning = false;
		while (!isFinished) {
	...
	// Find a single face using the Haar detector
	{
		 ...
		 * debugCanvas.showImage(grayIm); debugCanvas.waitKey(0);
		 */
	 ...
	 */
	{
		...
		synchronized (faceRect) {
			...
			try {
				clipIm = im.getSubimage(faceRect.x, faceRect.y, faceRect.width,
						faceRect.height);
	...
	/*
	 * resizes to at least a standard size, converts to grayscale, clips to an
	 * exact size, then saves in a standard location
	 */
	{
		long startTime = System.currentTimeMillis();
		...
		saveImage(faceIm, FACE_DIR + "/" + personFileName + fileCount + ".png");
		fileCount++;
