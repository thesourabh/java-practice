...
public class FileUtils {
	...
	// return all the names of the training image files + their paths
	{
		...
		if (fnms == null) {
			...
			System.out.println(TRAINING_DIR + " contains no " + " " + FILE_EXT
					+ " files");
			return null;
		} else
	 ...
	 */
	{
		...
		for (int row = 0; row < egFacesArr.length; row++) {
			String fnm = EIGENFACES_DIR + File.separator + EIGENFACES_PREFIX
					+ row + FILE_EXT;
	...
	// create a new directory or delete the contents of an existing one
	{
		...
		if (dirF.isDirectory()) {
			System.out.println("Directory: " + dir
					+ " already exists; deleting its contents");
	...
	private static void deleteFile(File f) {
		if (f.isFile()) {
			...
			/*
			 * if(deleted) System.out.println("  deleted: "+ f.getName() );
			 */
		}
	} // end of deleteFile()
	 ...
	 */
	{
		...
		for (int i = 0; i < ims.length; i++) {
			String fnm = RECON_DIR + File.separator + RECON_PREFIX + i
					+ FILE_EXT;
